...
import {
  ...
  hasProhibitedPersonalMedicalClaim,
...
export function evaluateCandidate(candidate, evidence, index) {
  ...
  if (hasPattern(ALWAYS_PROHIBITED_PATTERNS, allText)
    ...
    || hasProhibitedCredentialClaim([summary.title, summary.metaDescription, summary.visibleText])
...
export function validateDraftAgainstEvidence(draft, acceptedEvidence) {
  ...
  if (hasPattern(ALWAYS_PROHIBITED_PATTERNS, allClaims)
    ...
    || hasProhibitedPersonalMedicalClaim(proseItems)
    || hasProhibitedCredentialClaim(proseItems, true)
