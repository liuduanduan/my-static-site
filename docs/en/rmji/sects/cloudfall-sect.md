---
title: "Cloudfall Sect"
zh_name: "落云宗"
en_name: "Cloudfall Sect"
pinyin: "Luoyun Zong"
category: "Sects & Factions"
arc: "Mortal Realm middle-late arc"
check_status: "high"
description: "An important later Mortal Realm sect node in Han Li route."
---

# Cloudfall Sect / 落云宗

<div class="term-meta">
  <div><strong>Chinese</strong><span>落云宗</span></div>
  <div><strong>English</strong><span>Cloudfall Sect</span></div>
  <div><strong>Pinyin</strong><span>Luoyun Zong</span></div>
  <div><strong>Topic</strong><span>Sects & Factions</span></div>
</div>

## Summary

An important later Mortal Realm sect node in Han Li route.

## Role in RMJI

It should link with the return to Heavenly South, the Great Jin route, and Han Li changing status.

## Curation Notes

- Topic lane: Sects & Factions.
- Reading focus: Read it through regional order, resource allocation, mission systems, pursuit pressure, or political alliances.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 落云宗 |
| English name | Cloudfall Sect |
| Pinyin | Luoyun Zong |
| Topic | [Sects & Factions](/en/rmji/sects/) |
| Main arc | Mortal Realm middle-late arc |
| Check status | high |

## Related Entries

- [Return to Heavenly South / 返回天南](/en/rmji/timeline/return-to-heavenly-south)
- [Great Jin Journey / 大晋之行](/en/rmji/timeline/great-jin-journey)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
