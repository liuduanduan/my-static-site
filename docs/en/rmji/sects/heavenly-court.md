---
title: "Heavenly Court"
zh_name: "天庭"
en_name: "Heavenly Court"
pinyin: "Tianting"
category: "Sects & Factions"
arc: "Immortal World Arc"
check_status: "high"
description: "A core sequel faction representing higher order, rule, and law-based authority."
---

# Heavenly Court / 天庭

<div class="term-meta">
  <div><strong>Chinese</strong><span>天庭</span></div>
  <div><strong>English</strong><span>Heavenly Court</span></div>
  <div><strong>Pinyin</strong><span>Tianting</span></div>
  <div><strong>Topic</strong><span>Sects & Factions</span></div>
</div>

## Summary

A core sequel faction representing higher order, rule, and law-based authority.

## Role in RMJI

It should serve as a hub for immortal-world politics and link strongly with Reincarnation Palace and Dao Ancestor systems.

## Curation Notes

- Topic lane: Sects & Factions.
- Reading focus: Read it through regional order, resource allocation, mission systems, pursuit pressure, or political alliances.
- Series placement: This entry mainly supports the Immortal World Arc, where resource cultivation expands into laws, immortal factions, and Dao-level pressure.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 天庭 |
| English name | Heavenly Court |
| Pinyin | Tianting |
| Topic | [Sects & Factions](/en/rmji/sects/) |
| Main arc | Immortal World Arc |
| Check status | high |

## Related Entries

- [Reincarnation Palace / 轮回殿](/en/rmji/sects/reincarnation-palace)
- [Dao Ancestor System / 道祖体系](/en/rmji/laws/dao-ancestor-system)
- [Heavenly Court Territory / 天庭统辖区](/en/rmji/regions/heavenly-court-territory)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
