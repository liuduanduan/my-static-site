---
title: "Locust Race"
zh_name: "螟虫族"
en_name: "Locust Race"
pinyin: "Mingchong Zu"
category: "Races & Peoples"
arc: "Late Spirit Realm arc"
check_status: "high"
description: "A late Spirit Realm insect-race threat thread with strong pressure."
---

# Locust Race / 螟虫族

<div class="term-meta">
  <div><strong>Chinese</strong><span>螟虫族</span></div>
  <div><strong>English</strong><span>Locust Race</span></div>
  <div><strong>Pinyin</strong><span>Mingchong Zu</span></div>
  <div><strong>Topic</strong><span>Races & Peoples</span></div>
</div>

## Summary

A late Spirit Realm insect-race threat thread with strong pressure.

## Role in RMJI

It connects to the mother of locusts, racial crisis, and realm-scale disaster.

## Curation Notes

- Topic lane: Races & Peoples.
- Reading focus: Read it through ecology, hierarchy, resource habits, cross-realm conflict, and relations with human cultivators.
- Series placement: This entry belongs mostly to the Spirit Realm transition, where Han Li faces racial politics, higher materials, and broader realm-scale conflict.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 螟虫族 |
| English name | Locust Race |
| Pinyin | Mingchong Zu |
| Topic | [Races & Peoples](/en/rmji/races/) |
| Main arc | Late Spirit Realm arc |
| Check status | high |

## Related Entries

- [Mother of Locusts / 螟虫之母](/en/rmji/races/mother-of-locusts)
- [Spirit Realm / 灵界](/en/rmji/regions/spirit-realm)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
