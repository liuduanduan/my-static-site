---
title: "Nascent-Soul Formation Pill"
zh_name: "化婴丹"
en_name: "Nascent-Soul Formation Pill"
pinyin: "Huaying Dan"
category: "Elixirs & Materials"
arc: "Mortal Realm middle-late arc"
check_status: "medium"
description: "A high-value pill thread associated with attempting the Nascent Soul level."
---

# Nascent-Soul Formation Pill / 化婴丹

<div class="term-meta">
  <div><strong>Chinese</strong><span>化婴丹</span></div>
  <div><strong>English</strong><span>Nascent-Soul Formation Pill</span></div>
  <div><strong>Pinyin</strong><span>Huaying Dan</span></div>
  <div><strong>Topic</strong><span>Elixirs & Materials</span></div>
</div>

## Summary

A high-value pill thread associated with attempting the Nascent Soul level.

## Role in RMJI

It should connect with Nascent Soul, sect trade, auctions, and high-level materials.

## Curation Notes

- Topic lane: Elixirs & Materials.
- Reading focus: Read it through the resource chain: herb source, formula value, breakthrough use, trade value, and scarcity.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 化婴丹 |
| English name | Nascent-Soul Formation Pill |
| Pinyin | Huaying Dan |
| Topic | [Elixirs & Materials](/en/rmji/elixirs/) |
| Main arc | Mortal Realm middle-late arc |
| Check status | medium |

## Related Entries

- [Nine-Bend Spirit Ginseng / 九曲灵参](/en/rmji/elixirs/nine-bend-spirit-ginseng)
- [Spirit Stone / 灵石](/en/rmji/elixirs/spirit-stone)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
