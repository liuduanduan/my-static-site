---
title: "Foundation Establishment Pill"
zh_name: "筑基丹"
en_name: "Foundation Establishment Pill"
pinyin: "Zhuji Dan"
category: "Elixirs & Materials"
arc: "Mortal Realm early arc"
check_status: "high"
description: "A key pill for Qi Refining cultivators attempting Foundation Establishment, tied to sect allocation and lower-level competition."
---

# Foundation Establishment Pill / 筑基丹

<div class="term-meta">
  <div><strong>Chinese</strong><span>筑基丹</span></div>
  <div><strong>English</strong><span>Foundation Establishment Pill</span></div>
  <div><strong>Pinyin</strong><span>Zhuji Dan</span></div>
  <div><strong>Topic</strong><span>Elixirs & Materials</span></div>
</div>

## Summary

A key pill for Qi Refining cultivators attempting Foundation Establishment, tied to sect allocation and lower-level competition.

## Role in RMJI

It captures the early ecology of scarcity, limited chances, and breakthrough thresholds.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 筑基丹 |
| English name | Foundation Establishment Pill |
| Pinyin | Zhuji Dan |
| Topic | [Elixirs & Materials](/en/rmji/elixirs/) |
| Main arc | Mortal Realm early arc |
| Check status | high |

## Related Entries

- [Yellow Maple Valley / 黄枫谷](/en/rmji/sects/huangfeng-valley)
- [Spirit Stone / 灵石](/en/rmji/elixirs/spirit-stone)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
