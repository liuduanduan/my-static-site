---
title: "Dao Pill"
zh_name: "道丹"
en_name: "Dao Pill"
pinyin: "Dao Dan"
category: "Elixirs & Materials"
arc: "Immortal World Arc"
check_status: "needs chapter check"
description: "A sequel-era pill concept tied to law cultivation, dao comprehension, and high-level advancement."
---

# Dao Pill / 道丹

<div class="term-meta">
  <div><strong>Chinese</strong><span>道丹</span></div>
  <div><strong>English</strong><span>Dao Pill</span></div>
  <div><strong>Pinyin</strong><span>Dao Dan</span></div>
  <div><strong>Topic</strong><span>Elixirs & Materials</span></div>
</div>

## Summary

A sequel-era pill concept tied to law cultivation, dao comprehension, and high-level advancement.

## Role in RMJI

This batch creates a hub; later passes should split specific Dao Pill types and their refining systems.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 道丹 |
| English name | Dao Pill |
| Pinyin | Dao Dan |
| Topic | [Elixirs & Materials](/en/rmji/elixirs/) |
| Main arc | Immortal World Arc |
| Check status | needs chapter check |

## Related Entries

- [Three Thousand Great Daos / 三千大道](/en/rmji/laws/three-thousand-daos)
- [Dao Ancestor System / 道祖体系](/en/rmji/laws/dao-ancestor-system)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
