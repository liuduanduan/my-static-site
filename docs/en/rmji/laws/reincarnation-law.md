---
title: "Law of Reincarnation"
zh_name: "轮回法则"
en_name: "Law of Reincarnation"
pinyin: "Lunhui Faze"
category: "Laws & Dao"
arc: "Immortal World core"
check_status: "high"
description: "A high-level law tied to Reincarnation Palace, souls, life-death cycles, and identity mysteries."
---

# Law of Reincarnation / 轮回法则

<div class="term-meta">
  <div><strong>Chinese</strong><span>轮回法则</span></div>
  <div><strong>English</strong><span>Law of Reincarnation</span></div>
  <div><strong>Pinyin</strong><span>Lunhui Faze</span></div>
  <div><strong>Topic</strong><span>Laws & Dao</span></div>
</div>

## Summary

A high-level law tied to Reincarnation Palace, souls, life-death cycles, and identity mysteries.

## Role in RMJI

Together with time law, it forms one of the sequel most important philosophical conflicts.

## Curation Notes

- Topic lane: Laws & Dao.
- Reading focus: Read it through its links to realms, spiritual domains, Dao Ancestors, and high-level sequel combat.
- Series placement: This entry mainly supports the Immortal World Arc, where resource cultivation expands into laws, immortal factions, and Dao-level pressure.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 轮回法则 |
| English name | Law of Reincarnation |
| Pinyin | Lunhui Faze |
| Topic | [Laws & Dao](/en/rmji/laws/) |
| Main arc | Immortal World core |
| Check status | high |

## Related Entries

- [Reincarnation Palace / 轮回殿](/en/rmji/sects/reincarnation-palace)
- [Weeping Soul Beast / 啼魂兽](/en/rmji/artifacts/weeping-soul-beast)
- [Soul-Nurturing Wood / 养魂木](/en/rmji/elixirs/soul-nurturing-wood)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
