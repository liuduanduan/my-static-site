---
title: "Fireball Technique"
zh_name: "火弹术"
en_name: "Fireball Technique"
pinyin: "Huodan Shu"
category: "Techniques & Abilities"
arc: "Early Mortal Realm arc"
check_status: "high"
description: "One of the most direct low-level attack spells in the Everlasting Spring Art system, showing basic Qi Refining combat."
---

# Fireball Technique / 火弹术

<div class="term-meta">
  <div><strong>Chinese</strong><span>火弹术</span></div>
  <div><strong>English</strong><span>Fireball Technique</span></div>
  <div><strong>Pinyin</strong><span>Huodan Shu</span></div>
  <div><strong>Topic</strong><span>Techniques & Abilities</span></div>
</div>

## Summary

One of the most direct low-level attack spells in the Everlasting Spring Art system, showing basic Qi Refining combat.

## Role in RMJI

It grounds early combat in concrete spellcraft instead of only realm differences.

## Curation Notes

- Topic lane: Techniques & Abilities.
- Reading focus: Read it through cultivation efficiency, combat style, survival value, or law comprehension.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 火弹术 |
| English name | Fireball Technique |
| Pinyin | Huodan Shu |
| Topic | [Techniques & Abilities](/en/rmji/techniques/) |
| Main arc | Early Mortal Realm arc |
| Check status | high |

## Related Entries

- [Everlasting Spring Art / 长春功](/en/rmji/techniques/changchun-gong)
- [Qi Refining Stage / 炼气期](/en/rmji/laws/qi-refining-stage)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
