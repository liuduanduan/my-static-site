---
title: "Magnetic Divine Light"
zh_name: "元磁神光"
en_name: "Magnetic Divine Light"
pinyin: "Yuanci Shenguang"
category: "Techniques & Abilities"
arc: "Mortal Realm middle-late arc"
check_status: "high"
description: "A special ability based on magnetic force, often used to restrain five-element powers and treasures."
---

# Magnetic Divine Light / 元磁神光

<div class="term-meta">
  <div><strong>Chinese</strong><span>元磁神光</span></div>
  <div><strong>English</strong><span>Magnetic Divine Light</span></div>
  <div><strong>Pinyin</strong><span>Yuanci Shenguang</span></div>
  <div><strong>Topic</strong><span>Techniques & Abilities</span></div>
</div>

## Summary

A special ability based on magnetic force, often used to restrain five-element powers and treasures.

## Role in RMJI

It shows RMJI combat is not just realm comparison, but also attribute counters, restraint, and battlefield conditions.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 元磁神光 |
| English name | Magnetic Divine Light |
| Pinyin | Yuanci Shenguang |
| Topic | [Techniques & Abilities](/en/rmji/techniques/) |
| Main arc | Mortal Realm middle-late arc |
| Check status | high |

## Related Entries

- [Law of Five Elements / 五行法则](/en/rmji/laws/five-elements-law)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
