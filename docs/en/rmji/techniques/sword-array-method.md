---
title: "Sword Formation Methods"
zh_name: "剑阵之道"
en_name: "Sword Formation Methods"
pinyin: "Jianzhen Zhidao"
category: "Techniques & Abilities"
arc: "Mortal to Spirit Realm"
check_status: "medium"
description: "A combat system formed by combining sets of flying swords with formation logic."
---

# Sword Formation Methods / 剑阵之道

<div class="term-meta">
  <div><strong>Chinese</strong><span>剑阵之道</span></div>
  <div><strong>English</strong><span>Sword Formation Methods</span></div>
  <div><strong>Pinyin</strong><span>Jianzhen Zhidao</span></div>
  <div><strong>Topic</strong><span>Techniques & Abilities</span></div>
</div>

## Summary

A combat system formed by combining sets of flying swords with formation logic.

## Role in RMJI

Later passes should split major named sword formations into separate pages; this first batch creates the hub.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 剑阵之道 |
| English name | Sword Formation Methods |
| Pinyin | Jianzhen Zhidao |
| Topic | [Techniques & Abilities](/en/rmji/techniques/) |
| Main arc | Mortal to Spirit Realm |
| Check status | medium |

## Related Entries

- [Azure Essence Sword Art / 青元剑诀](/en/rmji/techniques/qingyuan-sword-art)
- [Bamboo Cloudswarm Swords / 青竹蜂云剑](/en/rmji/artifacts/green-bamboo-cloudswarm-swords)
- [Formation Flags and Plates / 阵旗阵盘](/en/rmji/artifacts/formation-flags)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
