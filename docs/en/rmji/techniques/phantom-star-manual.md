---
title: "Phantom Star Manual"
zh_name: "幻辰宝典"
en_name: "Phantom Star Manual"
pinyin: "Huanchen Baodian"
category: "Techniques & Abilities"
arc: "Immortal World Arc"
check_status: "needs chapter check"
description: "An important sequel-era manual that can be read through time, illusion, and star imagery."
---

# Phantom Star Manual / 幻辰宝典

<div class="term-meta">
  <div><strong>Chinese</strong><span>幻辰宝典</span></div>
  <div><strong>English</strong><span>Phantom Star Manual</span></div>
  <div><strong>Pinyin</strong><span>Huanchen Baodian</span></div>
  <div><strong>Topic</strong><span>Techniques & Abilities</span></div>
</div>

## Summary

An important sequel-era manual that can be read through time, illusion, and star imagery.

## Role in RMJI

This seed entry records its system position first; later passes should add inheritance, users, and cross-art relations.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 幻辰宝典 |
| English name | Phantom Star Manual |
| Pinyin | Huanchen Baodian |
| Topic | [Techniques & Abilities](/en/rmji/techniques/) |
| Main arc | Immortal World Arc |
| Check status | needs chapter check |

## Related Entries

- [Law of Time / 时间法则](/en/rmji/laws/time-law)
- [Mantra Wheel Scripture / 真言化轮经](/en/rmji/techniques/mantra-wheel-scripture)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
