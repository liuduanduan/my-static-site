---
title: "Spirit Refining Art"
zh_name: "炼神术"
en_name: "Spirit Refining Art"
pinyin: "Lianshen Shu"
category: "Techniques & Abilities"
arc: "Immortal World Arc"
check_status: "high"
description: "A key sequel-era training direction focused on soul, divine sense, and mental carrying capacity."
---

# Spirit Refining Art / 炼神术

<div class="term-meta">
  <div><strong>Chinese</strong><span>炼神术</span></div>
  <div><strong>English</strong><span>Spirit Refining Art</span></div>
  <div><strong>Pinyin</strong><span>Lianshen Shu</span></div>
  <div><strong>Topic</strong><span>Techniques & Abilities</span></div>
</div>

## Summary

A key sequel-era training direction focused on soul, divine sense, and mental carrying capacity.

## Role in RMJI

It works best alongside time law, the Mantra Wheel Scripture, and Reincarnation Palace threads.

## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 炼神术 |
| English name | Spirit Refining Art |
| Pinyin | Lianshen Shu |
| Topic | [Techniques & Abilities](/en/rmji/techniques/) |
| Main arc | Immortal World Arc |
| Check status | high |

## Related Entries

- [Law of Time / 时间法则](/en/rmji/laws/time-law)
- [Mantra Wheel Scripture / 真言化轮经](/en/rmji/techniques/mantra-wheel-scripture)
- [Reincarnation Palace / 轮回殿](/en/rmji/sects/reincarnation-palace)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
