---
title: "Xiao Cuier"
zh_name: "小翠儿"
en_name: "Xiao Cuier"
pinyin: "Xiao Cuier"
category: "Characters"
arc: "Early Mortal Realm arc"
check_status: "medium"
description: "An early character tied to human ties, entrusted care, and non-sect side threads."
---

# Xiao Cuier / 小翠儿

<div class="term-meta">
  <div><strong>Chinese</strong><span>小翠儿</span></div>
  <div><strong>English</strong><span>Xiao Cuier</span></div>
  <div><strong>Pinyin</strong><span>Xiao Cuier</span></div>
  <div><strong>Topic</strong><span>Characters</span></div>
</div>

## Summary

An early character tied to human ties, entrusted care, and non-sect side threads.

## Role in RMJI

She adds the side of Han Li dealing with old ties and human obligations.

## Curation Notes

- Topic lane: Characters.
- Reading focus: Read the character as mentor, emotional karma, hostile pressure, companion support, or symbol of higher order.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 小翠儿 |
| English name | Xiao Cuier |
| Pinyin | Xiao Cuier |
| Topic | [Characters](/en/rmji/characters/) |
| Main arc | Early Mortal Realm arc |
| Check status | medium |

## Related Entries

- [Han Li / 韩立](/en/rmji/characters/han-li)
- [Yellow Maple Valley Disciple Group / 黄枫谷弟子群像](/en/rmji/characters/yellow-maple-disciples-group)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
