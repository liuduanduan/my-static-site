---
title: "Hong Fu"
zh_name: "红拂"
en_name: "Hong Fu"
pinyin: "Hongfu"
category: "Characters"
arc: "Early Mortal Realm arc"
timeline_node: "Mortal Realm 03 | Yue Seven Sects and Foundation Establishment"
check_status: "arc checked"
legacy_check_status: "medium"
description: "A high-level female cultivator related to Yellow Maple Valley, useful for completing the early sect character map."
---

# Hong Fu / 红拂

<div class="term-meta">
  <div><strong>Chinese</strong><span>红拂</span></div>
  <div><strong>English</strong><span>Hong Fu</span></div>
  <div><strong>Pinyin</strong><span>Hongfu</span></div>
  <div><strong>Topic</strong><span>Characters</span></div>
</div>

## Summary

A high-level female cultivator related to Yellow Maple Valley, useful for completing the early sect character map.

## Role in RMJI

This first character pass creates the index; later passes can add events and relationships.

## Curation Notes

- Topic lane: Characters.
- Chronology node: Mortal Realm 03 | Yue Seven Sects and Foundation Establishment.
- Reading focus: Read the character as mentor, emotional karma, hostile pressure, companion support, or symbol of higher order.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.

## Timeline Placement

| Field | Detail |
| --- | --- |
| Reading order | Mortal Realm -> Spirit Realm -> Immortal World |
| This entry belongs to | Mortal Realm 03 | Yue Seven Sects and Foundation Establishment |
| Major realm | Mortal Realm |
| Arc label | Early Mortal Realm arc |
| Chapter segment | Tainan gathering, Yellow Maple Valley, Blood Forbidden Land, and Yue Seven Sects segment |
| Why it is here | Tainan gathering, Yellow Maple Valley, Blood Forbidden Land, Foundation pills, Yue conflict pressure, and early sect ties. |


## Character File

| Field | Detail |
| --- | --- |
| Timeline node | Mortal Realm 03 | Yue Seven Sects and Foundation Establishment |
| First / major meeting | Completes early sect systems, foundation resources, emotional karma, and the green-bottle resource loop. |
| Cultivation at meeting / main stage | Early Mortal Realm arc realm details remain chapter-level work; currently located by arc and role. |
| Identity and faction | A high-level female cultivator related to Yellow Maple Valley, useful for completing the early sect character map. |
| Arts, laws, or treasures | Yellow Maple Valley |
| Relation to Han Li | This first character pass creates the index; later passes can add events and relationships. |
| Outcome / later thread | Located by arc; exact chapters and relationship changes can be refined later. |
| Age precision | about age 18-31 |
| Note | Original encyclopedia notes only; if cultivation changes, the meeting or main arc takes priority. |




## Chapter Check Ledger

| Field | Detail |
| --- | --- |
| Work | A Record of a Mortal's Journey to Immortality |
| Chapter segment | Tainan gathering, Yellow Maple Valley, Blood Forbidden Land, and Yue Seven Sects segment |
| First check point | Completes early sect systems, foundation resources, emotional karma, and the green-bottle resource loop. |
| Main place | Tainan gathering, Yellow Maple Valley, Yue Seven Sects, Blood Forbidden Land |
| People | Han Li, Nangong Wan, Yellow Maple peers, Yue Seven Sects disciples |
| Event | Completes early sect systems, foundation resources, emotional karma, and the green-bottle resource loop. |
| Materials / arts | Foundation pills, spirit herbs, green bottle, low-level tools, spirit stones |
| Han Li age estimate | about age 18-31 |
| Check status | arc checked |
| Legal sources | [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/), [Wuxiaworld English RMJI reading page](https://www.wuxiaworld.com/novel/rmji) |
| Note | This pass records chapter segments, event facts, and entity links without reproducing novel prose. |




## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 红拂 |
| English name | Hong Fu |
| Pinyin | Hongfu |
| Topic | [Characters](/en/rmji/characters/) |
| Timeline node | Mortal Realm 03 | Yue Seven Sects and Foundation Establishment |
| Main arc | Early Mortal Realm arc |
| Chapter check | arc checked |
| Legacy confidence | medium |

## Related Entries

- [Yellow Maple Valley / 黄枫谷](/en/rmji/sects/huangfeng-valley)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/1cd32a70713c621c1cdee58)
- [Wuxiaworld English RMJI reading page](https://www.wuxiaworld.com/novel/rmji)
- [Wuxiaworld English Immortal Realm reading page](https://www.wuxiaworld.com/novel/rmjiir)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
