---
title: "Bonded Artifact"
zh_name: "本命法宝"
en_name: "Bonded Artifact"
pinyin: "Benming Fabao"
category: "Artifacts & Companions"
arc: "Mortal to Spirit Realm"
timeline_node: "Spirit Realm 01 | Ascension landing and Heavenly Abyss City"
check_status: "arc checked"
legacy_check_status: "high"
description: "A core artifact type refined over time and linked to a cultivator mind and progression."
---

# Bonded Artifact / 本命法宝

<div class="term-meta">
  <div><strong>Chinese</strong><span>本命法宝</span></div>
  <div><strong>English</strong><span>Bonded Artifact</span></div>
  <div><strong>Pinyin</strong><span>Benming Fabao</span></div>
  <div><strong>Topic</strong><span>Artifacts & Companions</span></div>
</div>

## Summary

A core artifact type refined over time and linked to a cultivator mind and progression.

## Role in RMJI

Long-growth weapons such as the Bamboo Cloudswarm Swords express the value of bonded artifacts.

## Curation Notes

- Topic lane: Artifacts & Companions.
- Chronology node: Spirit Realm 01 | Ascension landing and Heavenly Abyss City.
- Reading focus: Read it through origin, refinement, material attribute, growth potential, and Han Li trump-card system.
- Series placement: This entry spans the Mortal-to-Spirit Realm transition, so it is useful for tracking how early tools grow into higher-realm systems.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.

## Timeline Placement

| Field | Detail |
| --- | --- |
| Reading order | Mortal Realm -> Spirit Realm -> Immortal World |
| This entry belongs to | Spirit Realm 01 | Ascension landing and Heavenly Abyss City |
| Major realm | Spirit Realm |
| Arc label | Mortal to Spirit Realm |
| Chapter segment | Mortal to Spirit Realm related chapter segment |
| Why it is here | Identity rebuilding after ascension, human-demon defenses, Heavenly Abyss City order, and higher-realm resources. |




## Crafting and Effect File

| Field | Detail |
| --- | --- |
| Timeline node | Spirit Realm 01 | Ascension landing and Heavenly Abyss City |
| Source / acquisition | Mortal to Spirit Realm related chapter segment |
| Materials / conditions | Specific materials pending chapter check; currently records known resource class and chain. |
| Method | Organized by acquisition, refinement conditions, and use. |
| Effect | Long-growth weapons such as the Bamboo Cloudswarm Swords express the value of bonded artifacts. |
| Users / related people | Han Li and related figures |
| Limits | Materials, formula, or refinement details remain subject to chapter-level checking; this page does not invent complete recipes. |
| Note | Records materials, method, and effects only; no novel prose is reproduced. |


## Chapter Check Ledger

| Field | Detail |
| --- | --- |
| Work | A Record of a Mortal's Journey to Immortality |
| Chapter segment | Mortal to Spirit Realm related chapter segment |
| First check point | First appearance of Bonded Artifact remains to be refined by chapter |
| Main place | Spirit Realm |
| People | Han Li and related figures |
| Event | A core artifact type refined over time and linked to a cultivator mind and progression. |
| Materials / arts | Bonded Artifact |
| Han Li age estimate | estimated by arc; chapter-level refinement pending |
| Check status | arc checked |
| Legal sources | [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/), [Wuxiaworld English RMJI reading page](https://www.wuxiaworld.com/novel/rmji) |
| Note | This is located to an arc or event segment; later passes can add exact chapter numbers and cross-links. |




## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 本命法宝 |
| English name | Bonded Artifact |
| Pinyin | Benming Fabao |
| Topic | [Artifacts & Companions](/en/rmji/artifacts/) |
| Timeline node | Spirit Realm 01 | Ascension landing and Heavenly Abyss City |
| Main arc | Mortal to Spirit Realm |
| Chapter check | arc checked |
| Legacy confidence | high |

## Related Entries

- [Bamboo Cloudswarm Swords / 青竹蜂云剑](/en/rmji/artifacts/green-bamboo-cloudswarm-swords)
- [Flying-Sword Control / 御剑术](/en/rmji/techniques/flying-sword-control)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/1cd32a70713c621c1cdee58)
- [Wuxiaworld English RMJI reading page](https://www.wuxiaworld.com/novel/rmji)
- [Wuxiaworld English Immortal Realm reading page](https://www.wuxiaworld.com/novel/rmjiir)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
