---
title: "Wind-Thunder Wings"
zh_name: "风雷翅"
en_name: "Wind-Thunder Wings"
pinyin: "Fenglei Chi"
category: "Artifacts & Companions"
arc: "Mortal Realm middle-late arc"
check_status: "high"
description: "A representative mobility treasure combining escape speed with wind and thunder attributes."
---

# Wind-Thunder Wings / 风雷翅

<div class="term-meta">
  <div><strong>Chinese</strong><span>风雷翅</span></div>
  <div><strong>English</strong><span>Wind-Thunder Wings</span></div>
  <div><strong>Pinyin</strong><span>Fenglei Chi</span></div>
  <div><strong>Topic</strong><span>Artifacts & Companions</span></div>
</div>

## Summary

A representative mobility treasure combining escape speed with wind and thunder attributes.

## Role in RMJI

It supports Han Li survival logic: fighting matters, but escape speed and risk avoidance matter just as much.

## Curation Notes

- Topic lane: Artifacts & Companions.
- Reading focus: Read it through origin, refinement, material attribute, growth potential, and Han Li trump-card system.
- Series placement: This entry belongs mostly to the Mortal Realm portion, where scarcity, sect systems, secret realms, and cautious accumulation define the story.
- Spoiler level: encyclopedia summary only; no chapter text is reproduced.



## Placement

| Field | Detail |
| --- | --- |
| Chinese name | 风雷翅 |
| English name | Wind-Thunder Wings |
| Pinyin | Fenglei Chi |
| Topic | [Artifacts & Companions](/en/rmji/artifacts/) |
| Main arc | Mortal Realm middle-late arc |
| Check status | high |

## Related Entries

- [Thunder Law / 雷电法则](/en/rmji/laws/thunder-law)

## Legal Reading

- [Qidian: A Record of a Mortal's Journey to Immortality](https://www.qidian.com/book/107580/)
- [Qidian: A Record of a Mortal's Journey to Immortality - Immortal World Arc](https://www.qidian.com/book/1010734492/)
- [WeRead: Immortal World Arc](https://weread.qq.com/web/bookDetail/4fe324e0811e754c1g014ab9)
- [Wuxiaworld English reading page](https://www.wuxiaworld.com/novel/rmji)

## Content Policy

This page is an original reference summary for terminology, worldbuilding, and reading context. It does not copy novel chapters.
